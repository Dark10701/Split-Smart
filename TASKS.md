...
- [x] **M6.5-16** Phone-based accounts: `User.phone` (unique, E.164; hand-written `20260717090000` migration — applied by `prisma:deploy` on next start), Indian-mobile normalization/validation shared in `@splitsmart/validation`, `phone_number` OIDC claim mapped into the user (backfilled on link), PATCH /me accepts phone (409 on duplicates), GDPR export/erasure updated.
- [x] **M6.5-17** Password accounts at the IdP boundary: dev issuer gains scrypt-hashed credential store (gitignored), `POST /register` (OTP-verified email + name + unique phone + password + optional UPI), `POST /login` (email OR mobile + password), `POST /password/change`. Web login is Sign in / Create account tabs. Production swaps the issuer for Auth0/Clerk — no API change.
- [x] **M6.5-18** Friends system: `Friendship` model (pending/accepted/blocked, unique pair), API module — search by phone/email/name (exact match on phone/email), send/accept/reject/cancel requests, remove, block/unblock, overview with states; 14 unit tests. Web Friends tab: requests, add-friend search modal, Pending/Accepted/Blocked states, balances merged in.
- [x] **M6.5-19** UPI payments: `/upi` My-UPI screen (personal QR + UPI ID + name + mobile, Download/Share QR, edit inline) — quick tile now opens it instead of the profile; `lib/upi.ts` provider seam (generic `upi://` chooser + named GPay/PhonePe/Paytm/BHIM schemes as one-line entries); friends rows get one-tap "Pay via UPI" with the exact net amount + a Record-settlement path.
